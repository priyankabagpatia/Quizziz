 ...
 */
public class check_android extends HttpServlet {
@Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Statement st = con.createStatement();
            for(int i=1;i<=10;i++){
                String ans = request.getParameter("android_option["+i+"]");
                String query = "Select * from questions_android where question_id = '"+i+"' ";
                ResultSet rs = st.executeQuery(query);
                if(rs.next()){
