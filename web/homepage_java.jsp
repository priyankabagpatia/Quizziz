    ...
    <div class="container" style="border: solid #000 1px; margin-top: 20px;margin-bottom: 20px;">
        ...
        <div class="row">
            ...
            try{
                ...
                Connection con = DriverManager.getConnection("jdbc:mysql://127.0.0.2:3306/onlinequiz","root","root");
                Statement st = con.createStatement();
                String query = "Select * from questions_java";
                ResultSet rs = st.executeQuery(query);
                %>
                <form action="check_java" method="POST">
                ...
                <div class="panel panel-primary col-lg-6 col-lg-offset-3">
                    <div class="panel-title">
                        ...
                        <div class="panel-body">
                            ...
                            <input type="radio" name="java_option[<%= rs.getString("question_id").toString() %>]" value="<%= rs.getString("option1").toString() %>" required/> <%= rs.getString("option1").toString() %><br/>
                            <input type="radio" name="java_option[<%= rs.getString("question_id").toString() %>]" value="<%= rs.getString("option2").toString() %>" required/> <%= rs.getString("option2").toString() %><br/>
                            <input type="radio" name="java_option[<%= rs.getString("question_id").toString() %>]" value="<%= rs.getString("option3").toString() %>" required/> <%= rs.getString("option3").toString() %><br/>
                            <input type="radio" name="java_option[<%= rs.getString("question_id").toString() %>]" value="<%= rs.getString("option4").toString() %>" required/> <%= rs.getString("option4").toString() %><br/>
